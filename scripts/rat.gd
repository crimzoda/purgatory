...
var damage_sound = load("res://sound/rat_damage.ogg")
var roaming_sound = load("res://sound/rat_roaming.ogg")
var death_sound = load("res://sound/rat_death.ogg")
...
func _physics_process(delta):
	...
	if following != null:
		...
		if state == "roaming":
			...
			# Light avoidance
			$Area3D/LightRayCast.target_position = (next_location - current_location)
			if  $Area3D/LightRayCast.get_collider() != null and $Area3D/LightRayCast.get_collider().get("type") == "light":
				$Area3D/LightRayCast.debug_shape_custom_color = Color.PURPLE
				$NavigationAgent3D.target_position = Vector3.ZERO
				velocity = Vector3.ZERO
			else:
				$Area3D/LightRayCast.debug_shape_custom_color = Color.RED
...
func damage(amount):
	if health > 0:
		health -= amount
		$AnimationPlayer.play("rat_damage")
		$AudioStreamPlayer3D.stream = damage_sound
		$AudioStreamPlayer3D.play()
	else:
		var carcass = carcass_scene.instantiate()
		carcass.texture = load("res://textures/rat_carcass.png")
