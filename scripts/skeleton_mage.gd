...
var roaming_sound = load("res://sound/skeleton_mage_roaming.ogg")
var damage_sound = load("res://sound/skeleton_damage.ogg")
var death_sound = load("res://sound/skeleton_mage_death.ogg")
...
func _physics_process(delta):
	...
	if following != null:
		...
		if $RayCast3D.is_colliding():
			...
			if hit_collider == following:
				...
				if global_transform.origin.distance_to($NavigationAgent3D.get_final_position()) < 5 or $NavigationAgent3D.target_position == Vector3.ZERO or int(velocity.length()) == 0 or (int(velocity.length()) == 0 and following != null):
					var target = Vector3(randi_range(global_transform.origin.x - 10, global_transform.origin.x + 10), global_transform.origin.y, randi_range(global_transform.origin.z - 10, global_transform.origin.z + 10))
					if target.distance_to(following.global_transform.origin) > 10:
	...
	$Label3D.text = "velocity: " + str(velocity.length())
...
func _on_fireball_timer_timeout():
	if can_shoot == true and $RayCast3D.get_collider() == following:
		var fireball_scene = load("res://objects/fireball.tscn")
		var fireball = fireball_scene.instantiate()
		fireball.damage = 10
		fireball.destination = following.global_transform.origin
...
func damage(amount):
	if health > 0:
		health -= amount
		$AnimationPlayer.play("skeleton_mage_damage")
		$AudioStreamPlayer3D.stream = damage_sound
		$AudioStreamPlayer3D.play()
	else:
		var carcass = carcass_scene.instantiate()
		carcass.texture = load("res://textures/skeleton_mage_carcass.png")
...
func _on_input_event(camera, event, position, normal, shape_idx):
	if event is InputEventMouseButton:
		...
		and Globals.player.can_hit == true):
				...
				print("damaging " + str(Globals.player.dmg))
				print("range " + str(Globals.player.range))
